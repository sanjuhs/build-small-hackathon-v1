...
POD_NAME="${POD_NAME:-fireboy-minicpm-lora-skill-param}"
...
EVAL_ROWS="${EVAL_ROWS:-384}"
LORA_RANK="${LORA_RANK:-8}"
LORA_ALPHA="${LORA_ALPHA:-16}"
LORA_DROPOUT="${LORA_DROPOUT:-0.05}"
LR_LORA="${LR_LORA:-1e-5}"
LR_HEAD="${LR_HEAD:-4e-5}"
RUN_EVAL="${RUN_EVAL:-1}"
HEAD_NAME="${HEAD_NAME:-fireboy_minicpm_vla_lora_skill_param_head}"
...
if [[ -z "${SEED_CHECKPOINT:-}" ]]; then
  SEED_CHECKPOINT="$ROOT/Fireboy-training-policy-vla/runpod-artifacts/checkpoints/fireboy_minicpm_vla_skill_param_head/minicpm_vla_skill_param_head.pt"
fi
if [[ ! -f "$SEED_CHECKPOINT" ]]; then
  echo "Missing seed skill-param checkpoint: $SEED_CHECKPOINT" >&2
  exit 1
fi
...
echo "Packing source..."
PACK="$ROOT/fireboy-vla-physics/build/runpod/fireboy-minicpm-lora-skill-param-source.tgz"
...
echo "Uploading source, all-skill rollout archive, and seed router checkpoint..."
"${SCP[@]}" "$PACK" "root@$SSH_IP:$REMOTE_ROOT/fireboy-minicpm-lora-skill-param-source.tgz"
"${SCP[@]}" "$VLA_ROLLOUT_ARCHIVE" "root@$SSH_IP:$REMOTE_ROOT/vla-allskill-rollouts.tgz"
"${SCP[@]}" "$SEED_CHECKPOINT" "root@$SSH_IP:$REMOTE_ROOT/seed_minicpm_vla_skill_param_head.pt"

echo "Training MiniCPM-V LoRA skill-param router on RunPod..."
"${SSH[@]}" "set -euo pipefail
cd $REMOTE_ROOT
rm -rf $REMOTE_PROJECT
tar --no-same-owner -xzf fireboy-minicpm-lora-skill-param-source.tgz
...
mkdir -p \$OUT_DIR
python $REMOTE_PROJECT/src/train_minicpm_vla_lora_skill_param_head.py \
  --manifest \$SKILL_MANIFEST \
  --seed-checkpoint $REMOTE_ROOT/seed_minicpm_vla_skill_param_head.pt \
  --out-dir \$OUT_DIR \
  ...
  --lr-lora $LR_LORA \
  --lr-head $LR_HEAD \
  | tee \$OUT_DIR/train_minicpm_vla_lora_skill_param_head.json
if [[ '$RUN_EVAL' == '1' ]]; then
  python $REMOTE_PROJECT/src/eval_minicpm_vla_skill_param_head.py \
    ...
    --embedding-cache \$OUT_DIR/minicpm_lora_skill_param_embedding_cache.npz \
    | tee \$OUT_DIR/eval_minicpm_vla_lora_skill_param_head.json
fi
...
tar -czf fireboy-minicpm-lora-skill-param-artifacts.tgz checkpoints/$HEAD_NAME
"

echo "Downloading MiniCPM LoRA skill-param artifacts..."
mkdir -p "$LOCAL_RUNPOD_ARTIFACT_DIR" "$LOCAL_CHECKPOINT_DIR"
"${SCP[@]}" "root@$SSH_IP:$REMOTE_PROJECT/build/fireboy-minicpm-lora-skill-param-artifacts.tgz" "$LOCAL_RUNPOD_ARTIFACT_DIR/fireboy-minicpm-lora-skill-param-artifacts.tgz"
tar -xzf "$LOCAL_RUNPOD_ARTIFACT_DIR/fireboy-minicpm-lora-skill-param-artifacts.tgz" -C "$LOCAL_RUNPOD_ARTIFACT_DIR"
cp "$LOCAL_RUNPOD_ARTIFACT_DIR/checkpoints/$HEAD_NAME/minicpm_vla_lora_skill_param_head.pt" "$LOCAL_CHECKPOINT_DIR/minicpm_vla_lora_skill_param_head.pt"
rm -rf "$LOCAL_CHECKPOINT_DIR/lora_adapter"
...
echo "Done."
echo "Pod: $POD_ID"
echo "Local checkpoint: $LOCAL_CHECKPOINT_DIR/minicpm_vla_lora_skill_param_head.pt"
