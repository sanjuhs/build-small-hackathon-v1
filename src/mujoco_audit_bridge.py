...
AUDIT_DIR = FIREBOY_VLA_ROOT / "build" / "audit"


def run_mujoco_policy_audit() -> dict[str, Any]:
    ...
    cmd = [
        str(python_bin),
        str(FIREBOY_VLA_ROOT / "src" / "audit_mujoco_policy.py"),
        "--out-dir",
        str(AUDIT_DIR),
    ]
    proc = subprocess.run(
        ...
        text=True,
        capture_output=True,
        timeout=float(os.getenv("TOYBOX_MUJOCO_AUDIT_TIMEOUT", "90")),
        check=False,
    )
    if proc.returncode != 0:
        raise RuntimeError((proc.stderr or proc.stdout or "unknown MuJoCo audit error")[-1400:])
